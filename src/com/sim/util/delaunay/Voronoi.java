...
public class Voronoi {
	...
	private void handleSiteEvent(Event cur) {
		...
		// Deal with the degenerate case where the first two points are at the
		// same y value
	...
	private void handleCircleEvent(CircleEvent ce) {
		...
		// Here we're trying to figure out if the org.ajwerner.voronoi.Voronoi
		// vertex we've found is the left
		// or right point of the new edge.
		// If the edges being traces out by these two arcs take a right turn
		// then we know
		...
		// So if it turns left, we know the next vertex will be below this
		// vertex
		// so if it's below and the slow is negative then this vertex is the
		// left point
	...
	// double topY = (e.p1.y == Double.POSITIVE_INFINITY) ? MAX_DIM : e.p1.y; //
	// HACK
	// // to
	// // draw
	// // from
